...
For a tag such as `v0.1.0-preview.1`:

| Output | Version |
| --- | --- |
| NuGet package and assembly informational version | `0.1.0-preview.1` (the assembly can also include the source commit) |
| Assembly and file version | `0.1.0.0`, generated by the .NET SDK |
| Gallery About screen | `0.1.0-preview.1`, read from the library assembly |
| iOS, Android, and macOS app display version | `0.1.0`, derived from the same version without the preview suffix |
...
The release workflow currently publishes the library package and GitHub Release. When building a gallery app separately, pass the same `-p:Version` value to give its manifest and assemblies the release version. The gallery shows `local` for an unversioned local build.

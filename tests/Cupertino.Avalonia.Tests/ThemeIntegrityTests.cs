...
public class ThemeIntegrityTests
{
    ...
    public void Adjacent_bottom_bar_surfaces_do_not_overlap(bool useStrip, int itemCount)
    {
        ...
        var hover = adjacent.GetVisualDescendants().OfType<Border>()
            .Single(border => border.Name == "PointerOverPill");
    ...
    public void A_segmented_drag_cancelled_by_vertical_scroll_clears_its_travel_pill()
    {
        ...
        Assert.Equal(track.Bounds.Height, indicator.Bounds.Height, 3);
        var fill = Assert.IsType<Border>(Assert.IsType<Border>(indicator).Child);
        Assert.True(fill.Opacity < 1, "the solid pill should fade as the glass lens appears");
        Assert.Equal(1, fill.Opacity + lens.Opacity, 3);
